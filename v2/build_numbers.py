...
Reads every CSV in data/ and every *.numbers.csv sidecar in
generated/figures/, then emits:

  generated/values.tex   -- one csname definition per (stem, key) pair
  generated/provenance.txt -- call sites in prose .tex -> CSV cell -> value

Run:
    python build_numbers.py [--paper-dir <path>]

Ported from hallulens/build_numbers.py — same contract, namespace
...
# ---------------------------------------------------------------------------
# CSV header parsing for data/ CSVs
# ---------------------------------------------------------------------------
...
def parse_data_csv_header(path: Path) -> DataCSVHeader:
    """Parse the comment block from a data/ CSV.
    ...
    if missing:
        raise ValueError(
            ...
            "Every CSV in data/ must start with the provenance header block."
        )
...
# ---------------------------------------------------------------------------
# Sidecar header parsing for generated/figures/*.numbers.csv
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Flatten data/ CSV into (stem, key) -> value
# ---------------------------------------------------------------------------
...
def emit_values_tex(cells: CellMap, out_path: Path) -> None:
    ...
    with open(out_path, "w") as fh:
        fh.write(f"% generated/values.tex\n")
...
def emit_provenance(cells: CellMap, prose_paths: list[Path], out_path: Path) -> list[dict]:
    ...
    with open(out_path, "w") as fh:
        fh.write(f"# generated/provenance.txt\n")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build generated/values.tex from CSVs")
    parser.add_argument(
        "--paper-dir",
        default=".",
    ...
    for sub in ("chapters", "appendices"):
        ...
        if sub_dir.exists():
            prose_paths.extend(sorted(sub_dir.glob("*.tex")))
    ...
    if unresolved:
        print(
            ...
            "Check generated/provenance.txt.",
            file=sys.stderr,
        )
