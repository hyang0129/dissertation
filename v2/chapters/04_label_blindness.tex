...
Within the dissertation's progression, this chapter examines the failure case:
an unlabeled objective may discard the label-relevant information before any
detector is applied. It establishes the conditional limit against which
\Cref{ch:domain-sensitivity-collapse,ch:hallucination-detection} study suppressed
and retained detection signals.
...
There is significant interest in unlabeled OOD detection for several reasons. A method that does not rely on labels can save significant labeling costs (\citealp{sehwag2021ssd}). It is also possible to skip training on the in-distribution data if such a model is generalizable (\citealp{wang2023clipn}). Self-supervised and unlabeled learning methods can also scale to much larger datasets, and these models must remain robust to OOD data. Recent work on unlabeled OOD detection, including \citet{sehwag2021ssd, tack2020csi, liu2023unsupervised, guille2024cadet, wang2023clipn}, promises to improve safety using only unlabeled data. These methods can achieve even greater performance than a simple supervised baseline \citep{hendrycks2016baseline}, suggesting that one could replace supervised training with self-supervised learning (SSL) for a safety-critical OOD detection task. This family of SSL OOD methods differs from traditional supervised OOD methods, including \citet{fort2021exploring}, by using only unlabeled data. The importance of labels is an active area of research in OOD detection \citep{du2024does, du2024and}.

When we view SSL from an information-theoretic perspective, the selection of features depends solely on the SSL objective and not on the labels. This, however, provides no guarantee that any features relevant for label prediction will be retained. Figure \ref{fig:grad} provides an example of how SSL features can be less effective for identifying a label. Our theory importantly shows that, when the label-relevant features are independent of the features relevant for the SSL algorithm's successful operation, OOD detection is guaranteed to fail due to what we call `label blindness' and that this label blindness occurs regardless of how one selects the ID dataset from the population of all data. Our experiments suggest that zero-shot OOD methods \citep{wang2023clipn, esmaeilpour2022zero} may also suffer from this issue. We show that unsupervised OOD detection methods behave in the same way as SSL in the context of information theory.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/gradcam_example_faces.png}
    \caption{An example failure case visualized using gradient heatmaps from an unlabeled, SimCLR-trained ResNet \citep{chen2020simple} and GradCAM \citep{selvaraju2017grad}. The OOD detection task is to detect held-out facial expressions. The method fails in the regime described by our theory: the representations do not exhibit a strong gradient in regions commonly associated with facial expressions, such as the eyebrows and mouth.}
    \label{fig:grad}
\end{figure}

However, benchmark construction can unintentionally avoid the label-blindness problem through its choice of OOD dataset. Existing methods generally consider ID and OOD data from different datasets, e.g., \citet{fort2021exploring}, \citet{sehwag2021ssd}, and \citet{hendrycks2019using}. In these benchmarks, there is no significant overlap between the ID and OOD input data, allowing OOD detection algorithms to succeed on features independent of the label. To address this issue and test for label blindness, we introduce the Adjacent OOD detection task, which evaluates OOD detection algorithms when the OOD and ID input data overlap substantially. We also prove that one cannot guarantee that a real-world system will never encounter OOD input data that significantly overlaps its ID input data.
...
\item \textbf{Adjacent OOD detection benchmarks.} We introduce the concept of bootstrapping without replacement of the ID labels to create the Adjacent OOD detection task. To the authors' knowledge, this task has not appeared in prior OOD detection research. It evaluates OOD detection when there is significant overlap between OOD and ID data. We also theoretically prove that overlapping OOD and ID data is possible in every real-world dataset.
...
% These shared definitions also appear in Chapter 2. They remain here so this
% publication-based chapter is self-contained.
\begin{definition}
...
Information bottleneck optimization can be expressed as minimizing the representation's complexity via $I(\rvx;\rvz)$ while maximizing its utility $I(\rvz;\rvy)$. This results in the information-theoretic loss function below, where $\beta$ controls the trade-off between complexity and utility \citep{shwartz2023compress}. In practice, learning $\rvz$ without $\rvy$ requires a surrogate task $\rvy_{s}$, e.g., \citet{chen2020simple}, with the loss defined as:
\begin{align}
\mathcal{L}=I(\rvx ; \rvz)-\beta I(\rvz ;\rvy).
\end{align}
The primary task $\rvy$ may equal the SSL task $\rvy_{s}$. In that case, compression toward the minimal sufficient statistic still occurs. This is important because unsupervised methods for deep neural networks (DNNs) use a surrogate task $\rvy_{u}$ to train the DNN's weights. Thus, if the primary task for an unsupervised learning method equals its surrogate task, it behaves identically to SSL from the perspective of information theory.
...
However, superfluous information is not affected by the labels of the primary task, only by $\rvx$ and $\rvy_{s}$. Using information theory, we can show that any SSL OOD detection algorithm will fail when the surrogate task $\rvy_{s}$ is independent of the labels in the in-distribution dataset. This applies to unsupervised OOD detection algorithms that also use a surrogate task.
...
We can use Fano's inequality to extend our understanding of strict label blindness to situations where the variables are not fully independent. The lower bound for prediction error is defined by the entropy of the target label $\rvy$ less the mutual information between the input $\rvx$ and target label, as shown in Theorem \ref{fano}. Under strict label blindness, when $I(\rvx;\rvy) = 0$, the lower bound for error is at its maximum. When $I(\rvx;\rvy) \approx 0$, the lower bound for error is large enough to be unreliable. We refer to this condition as \textbf{Approximate Label Blindness} and conduct experiments to evaluate it. Unless specified as strict, label blindness refers to the approximate case.
...
Our work applies to deep neural networks (DNNs) trained without labels for the purpose of OOD detection. The key assumption of information bottleneck compression is generally applicable to DNNs \citep{shwartz2017opening}. Regardless of other assumptions, such as the multi-view assumption, an information bottleneck DNN trained without labels will still compress data irrelevant to its loss objective, even if that data is relevant for its intended task. It does not matter what task the training process was originally designed for because the unlabeled training process ultimately follows its own learning objective. For any learning objective, there will exist an independent feature unless compression is not possible, as in $I(\rvx;\rvy) = I(\rvx;\rvx)$. If there is compression, then there exist labels for which OOD detection failure is guaranteed.
...
One logical consequence of Corollary \ref{mainbodyfailood} is that one cannot avoid failure due to label blindness by selecting different labels for one's ID set, so long as the label selection is independent from the learning objective. To test any OOD detection algorithm for label blindness failure, this simply entails selecting different labels for one's ID set. To construct this benchmark, we randomly sample labels to be considered ID and other labels to be considered OOD. This is similar to bootstrapping, but without replacement. If an OOD detection algorithm is `approximately label blind', its average OOD detection performance across the samples should be poor. We refer to this as the \textbf{Adjacent OOD Detection Benchmark}.
...
While methods that utilize only unlabeled data, such as \citet{sehwag2021ssd, liu2023unsupervised, guille2024cadet}, show promising results on both near- and far-OOD tasks, their performance on Adjacent OOD detection depends on the mutual information between the learned representation and the ID labels. Our theoretical work suggests that such methods will perform poorly if the surrogate task is independent of the labels.

The Adjacent OOD benchmark can also evaluate zero-shot OOD detection methods. Although our theoretical work does not extend to pretraining that uses labels, performance remains important when OOD data overlaps ID data.

\section{Experimental Results}

We conduct the following experiments to verify the existence of label blindness in unlabeled OOD detection methods. All hyperparameters and configurations were the best-performing settings from their respective original paper implementations, unless noted otherwise. Experiments are repeated $3$ times.

\subsection{Experimental Setup}

\paragraph{Supervised Baseline.}
We use Maximum Softmax Probability (MSP) \citep{hendrycks2016baseline} as our supervised baseline. We augment the training data using random rotation, horizontal flip, random crop, grayscale, and color jitter. Images are resized to $64\times64$. We train using stochastic gradient descent with momentum and a cosine annealing learning schedule. We train for $10$ warm-up epochs followed by $150$ regular epochs, selecting the weights with the highest validation accuracy. We use a standard ResNet-50 architecture.

\paragraph{Self-Supervised Baselines.}
We use two SSL methods to evaluate how representations are learned, SimCLR \citep{chen2020simple} and Rotation Loss (RotLoss) \citep{hendrycks2019using}. Images are resized to $64\times64$ for both cases. For SimCLR, we augment the training data using random rotation, horizontal flip, random crop, grayscale, and color jitter. For Rotation Loss, we use only random crop and horizontal flip. We train a standard ResNet-50 using stochastic gradient descent with momentum and a cosine annealing learning schedule. Training runs for $10$ warm-up epochs followed by $500$ regular epochs. We use a KNN classifier at the end of each epoch and select the weights that yield the best validation KNN accuracy.

To evaluate OOD performance, we use two methods to score each sample, SSD \citep{sehwag2021ssd} and KNN, following \citet{sun2022out}. SSD uses the sample's Mahalanobis distance from the center of the in-distribution training representations. KNN uses the Euclidean distance between the test representation and its $N$th nearest neighbor among the in-distribution training representations. Both are distance-based OOD detection methods commonly used with representation learning. We use the representations described in the previous paragraph.

\paragraph{Unsupervised Baseline.}
To consider how an unsupervised OOD detection method functions, we evaluate the diffusion-inpainting method proposed by \citet{liu2023unsupervised} using code from the repository linked in that paper. We use the training configuration that generated the paper's main results: an alternating $8 \times 8$ checkerboard mask, an LPIPS distance metric for the OOD score, and $10$ reconstructions per image. We modify only the input image size to $64\times64$ for all datasets and run additional experiments using the alternative MSE distance metric. This method is representative of other generative methods, such as \citet{xiao2020likelihood}.

\paragraph{Zero-shot Baseline.}
To consider how well zero-shot learning algorithms perform, we evaluate the CLIPN model presented by \citet{wang2023clipn}. We use the pretrained weights provided in the paper's repository and perform zero-shot OOD detection on our Adjacent OOD benchmark. We evaluate CLIPN's performance using $3$ algorithms from that paper: Maximum Softmax Probability, Compete to Win (CTW), and Agree to Differ (ATD).

\subsection{Adjacent OOD Datasets}

To create the Adjacent OOD detection task, we randomly split $25$\% of all classes into the OOD set and retain $75$\% as the ID set. We also repeat our experiments three times with different seeds to account for different splits of the ID and OOD set. Only ICML Facial Expressions has a major class imbalance in one of its seven classes.
...
Experimental results for Adjacent OOD are presented in Table \ref{tab:results}. The supervised baseline performs better than most unlabeled methods on the Adjacent OOD detection task. When unlabeled methods perform no better than random guessing, the learned representation likely contains little information about the semantic label. This contrasts with improvements reported in unlabeled OOD papers \citep{sehwag2021ssd, hendrycks2019using, liu2023unsupervised}: on this benchmark, the unlabeled methods perform significantly worse than a simple MSP baseline.

The zero-shot CLIPN method performs well when the label text's usage in pretraining is similar to the label text's usage in the ID data. In the case of the Cars dataset, the pretraining dataset CC3M \citep{sharma2018conceptual} contains many images captioned with the make and model of cars, resulting in good performance. The Food dataset also sees similar label usage in the pretraining set. However, the Faces dataset's labels are not aligned. For example, there are multiple images associated with the emotion angry that do not contain a human face, such as an image of an angry fist. When there is little or no mutual information between the pretraining data and the ID labels, zero-shot methods will perform poorly in OOD detection tasks.

\begin{table}[h]
\caption{Results from experiments across all datasets and methods. Unlabeled methods perform poorly in adjacent OOD detection. CLIPN performance is due to labels present in the pretraining dataset. Higher AUROC and lower FPR are better. All values are AUROC / FPR95 (\%), mean $\pm$ std over $3$ seeds.}
\vspace{2mm}
\centering
...
We observe competitive OOD performance from unlabeled SimCLR relative to supervised MSP on the CIFAR-10 and CIFAR-100 datasets. This is likely because SimCLR learns more of the relevant features in these datasets and because the classes are more visually dissimilar, resulting in less overlap between OOD and ID data. We also find strong far-OOD performance for SimCLR-based detection, consistent with work that evaluates unlabeled OOD methods on far-OOD benchmarks \citep{sehwag2021ssd, tack2020csi, liu2023unsupervised, guille2024cadet, wang2023clipn}.
...
In the dissertation's common framework, label blindness is the case in which the
required distinction is absent from the learned representation. The next chapter
examines a different failure: class supervision is available, but single-domain
training suppresses domain-sensitive directions that can still be restored.
